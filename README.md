# A Case Study: Analýza vývoje mezd a cen potravin v ČR v letech 2000 - 2021
...
- normalizace jednotek (kg, l, ks, g) - všechny hodnoty převedeny na 1 kg/l
- rozdílná granularita dat (např. kvartální vs. roční, sběr dat dle regionů)
...
## Vizualizace úkolů v Power BI:
- Vizualizace nad agregovanými daty (*t_{jmeno}_{prijmeni}_project_SQL_primary_final* v PBI přejmenována na WagesPrices a *t_{jmeno}_{prijmeni}_project_SQL_secondary_final* přejmenována na GDP).
- Vytvoření kalendářové tabulky (CalendarTab).
- ER diagram
...
- v projektu byly užity analytické funkce: LAG(), LEAD(), MIN(), MAX(), FIRST_VALUE(), LAST_VALUE(), DENSE_RANK()
- metrika CAGR - *Compound Annual Growth Rate* (*Složená roční míra růstu*), YoY - *Year-over-Year*, funkce POWER() nejen v SQL ale následně i pro vizualizaci do jazyka DAX
- INNER JOIN, LEFT JOIN, agregační funkce AVG(), CASE
- ošetřen případ dělení nulou funkcí NULLIF(), zaokrouhlení všech hodnot na 2 desetinná místa funkcí ROUND()
...
* Analýza neprokázala jednoznačný vztah mezi růstem HDP (výraznější růst HDP stanoven od 3%) a růstem mezd či cen potravin. Růst mezd sice s HDP částečně koreluje, ale bez výrazné závislosti a často se zpožděním. U cen potravin se žádná systematická vazba neprokázala (pro korelaci není dostatečné množství dat (2000-2006 NULL hodnoty pro průměrné ceny potravin, totéž od r. 2019 do 2021)). Výsledky naznačují, že na vývoj mezd a zejména cen potravin působí i další faktory mimo HDP
